...
window.addEventListener(`keydown`,function(evt){
  if(evt.keyCode===27){
    if(document.querySelectorAll('.popup'))
...
if(button){
  ...
  if(popup){
  let name=popup.querySelector('[name=name-field]')
  ...
  button.addEventListener(`click`,function(evt){
    ...
    if(localStorage.getItem(`name`)){
      ...
      if(storage=localStorage.getItem(`email`)){
  ...
  if(!name.value||!email.value||!text.value){
    evt.preventDefault();
    this.classList.remove(`modal-eror`)
...
button.addEventListener(`click`,function(evt){
  ...
  document.querySelector(`.modal-map`).classList.add(`modal-show`)
})}
